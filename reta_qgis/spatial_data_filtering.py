...
class DataCleaner:
    def __init__(self, dataframe, crs=None, feedback=None, verbose=True):
        """
        ...
        verbose: True to print detailed internal logs for validation.
        """
    ...
    def calculate_geometry(self):
        ...
        # This is the "Universal Metric Path" - no more back-and-forth conversions.
        if '_x_m' in self.df.columns and '_y_m' in self.df.columns:
            self.log("  [Geometry] Using pre-calculated metric coordinates (_x_m, _y_m).")
            x = self.df['_x_m'].values
            y = self.df['_y_m'].values
        elif self._crs and CRS_UTILS_AVAILABLE:
            self.log(f"  [Geometry] Projecting coordinates to {self._crs}...")
            xm, ym = points_to_meters(self.df['lon'].values, self.df['lat'].values, self._crs)
            x, y = xm, ym
            self.df['_x_m'] = xm
            self.df['_y_m'] = ym
        else:
            self.log("  [Geometry] No projection available. Using approximate degree-to-meter scaling.")
            base_lat = self.df['lat'].min()
            base_lon = self.df['lon'].min()
            R = 6371000.0 * np.pi / 180.0
            scale_y = R
            scale_x = R * np.cos(np.radians(base_lat))
            x = (self.df['lon'] - base_lon).values * scale_x
            y = (self.df['lat'] - base_lat).values * scale_y
            self.df['_x_m'] = x
            self.df['_y_m'] = y
            
        prev_x = pd.Series(x).shift(1).values
        prev_y = pd.Series(y).shift(1).values

        # Meters and Bearing using Euclidean math on local projection
        dx = x - prev_x
        dy = y - prev_y
        self.df['dist_m'] = np.sqrt(dx*dx + dy*dy)
        ...
        if time_col:
            ...
            if not np.issubdtype(self.df[time_col].dtype, np.number):
                try:
                    self.df['_temp_time'] = pd.to_datetime(self.df[time_col], errors='coerce').view('int64') // 10**9
                    if not self.df['_temp_time'].isna().all():
                        time_col = '_temp_time'
                        self.log(f"  [Geometry] Parsed datetime column '{col}' into seconds.")
                except:
                    pass
    ...
    def identify_transects_v7(self, turn_threshold=45, min_len_m=30):
        ...
        if self.verbose:
            ...
            self.log(f"  [Transects] Found {n_valid} valid passes and {n_short} short segments.")
    ...
    def identify_smart_turns(self, turn_threshold=45):
        ...
        for i in range(len(valid_tids) - 1):
            ...
            if is_u_turn:
                ...
                # Let's use position in the dataframe if possible?
                # or just loc
    ...
    def infer_swath_width(self):
        ...
        """
        if 'swath_width' in self.df.columns:
            inferred = self.df['swath_width'].median()
            if 2.0 < inferred < 40.0:
                self.log(f"  [Swath] Inferred from 'swath_width' column: {inferred:.2f}m")
                return inferred

        if 'Width' in self.df.columns:
            inferred = self.df['Width'].median()
            if 2.0 < inferred < 40.0:
                self.log(f"  [Swath] Inferred from 'Width' column: {inferred:.2f}m")
                return inferred

        if CRS_UTILS_AVAILABLE:
            area_m2 = area_m2_from_points(self.df['lon'].values, self.df['lat'].values, self._crs)
            total_dist_m = self.df['dist_m'].sum()
            if total_dist_m > 0:
                calc_swath = area_m2 / total_dist_m
                if 2.0 < calc_swath < 40.0:
                    self.log(f"  [Swath] Inferred using Area/Distance: {calc_swath:.2f}m")
                    return calc_swath
                else:
                    self.log(f"  [Swath] Inferred value {calc_swath:.2f}m outside reasonable range (2-40m).")

        self.log("  [Swath] All inference methods failed. Falling back to default 10.0m.")
        return 10.0

    def calculate_swath_width(self):
        if self.swath_width is None:
            self.swath_width = self.infer_swath_width()
    ...
    def calculate_auto_speed_limits(self):
        ...
        self.log(f"  [Auto-Limits] Speed limits inferred: {min_s:.2f} to {max_s:.2f} m/s")
        return float(min_s), float(max_s)

    def detect_overlaps_grid_v7(self, cell_size_m=1.0):
        swath = self.calculate_swath_width()
        
        # Golden Rule: Strictly use pre-calculated _x_m and _y_m
        if '_x_m' in self.df.columns and '_y_m' in self.df.columns:
            x_m = self.df['_x_m'].values - self.df['_x_m'].min()
            y_m = self.df['_y_m'].values - self.df['_y_m'].min()
        elif self._crs and CRS_UTILS_AVAILABLE:
            self.log("  [Overlap] Metric coordinates missing. Calculating for grid...")
            xm, ym = points_to_meters(self.df['lon'].values, self.df['lat'].values, self._crs)
            x_m = xm - xm.min()
            y_m = ym - ym.min()
        else:
            self.log("  [Overlap] Falling back to approximate scaling...")
            base_lat = self.df['lat'].min()
            R = 6371000.0 * np.pi / 180.0
            scale_y = R
            scale_x = R * np.cos(np.radians(base_lat))
            x_m = (self.df['lon'] - self.df['lon'].min()).values * scale_x
            y_m = (self.df['lat'] - self.df['lat'].min()).values * scale_y
        ...
        self.log(f"  [Overlap] Detected {sum(overlaps)} overlapping points.")
    ...
    def filter_overlap_speckles(self, min_size=5):
        ...
        if removed_count > 0:
            self.log(f"  [Speckle] Removed {removed_count} isolated overlap points.")
    ...
    def apply_filters_sequential(self, filters):
        ...
        """
        self.log(f"\n--- Starting V8 Sequential Filtering [Swath: {self.calculate_swath_width():.2f}m] ---")
        self.df['error_flag'] = 0
        var_col = filters.get('variable_col', 'yield') 
        
        # --- PHASE 1: OPERATIONAL ---
        self.log("Phase 1: Detecting Operational Errors...")
        # 1.1 Turns & Short
        op_count = 0
        if filters.get('filter_maneuvers', True):
             ...
             self.log(f"  [Op] Maneuvers/Turns: {self.df['is_turn'].sum()} turn, {self.df['is_short'].sum()} short points.")
        
        # 1.2 Overlaps
        if filters.get('filter_overlap', True):
        ...
        # 1.3 Speed & Accel
        if filters.get('filter_speed', True):
        ...
        if filters.get('filter_accel', True) and 'accel_mps2' in self.df.columns:
            self.df.loc[self.df['accel_mps2'] > 1.5, 'error_flag'] |= self.error_masks['Op: High Acceleration']
        
        # 1.4 Distance Jumps
        if filters.get('filter_dist_jump', True):
            median_dist = self.df['dist_m'].median()
            if median_dist > 0:
                jump_mask = self.df['dist_m'] > (5 * median_dist)
                self.df.loc[jump_mask, 'error_flag'] |= self.error_masks['Op: Distance Jump']
                if jump_mask.any(): self.log(f"  [Op] Distance Jumps: {jump_mask.sum()} points (> {5*median_dist:.1f}m).")
        ...
        if self.verbose:
            op_hits = (self.df['error_flag'] & op_mask_val) != 0
            self.log(f"Phase 1 Complete: {op_hits.sum()} points flagged as Operational Error.")

        # --- PHASE 2: GLOBAL ---
        self.log("Phase 2: Detecting Global Outliers...")
        ...
        if var_col in self.df.columns:
            ...
            if filters.get('use_aoi', False) and not val_series.empty:
                ...
                self.log(f"  [Gl] AOI Suggestion: {low_a:.2f} to {high_a:.2f} (margin: {margin})")

                # Sequential refinement: if use_aoi is ON, AOI overrides ANY manual param unless it's None in the UI
                # In QGIS, params usually have defaults (0, 2000). 
                # Excellence: If use_aoi is True, we prioritize AOI.
                min_y = low_a
                max_y = high_a
            
            self.log(f"  [Gl] Final Bounds: {min_y if min_y is not None else 'None'} to {max_y if max_y is not None else 'None'}")

            # Apply Limit Checks only to CLEAN points
            if min_y is not None:
                low_mask = op_mask & (self.df[var_col] < min_y)
                n_low = low_mask.sum()
                if n_low > 0: self.log(f"  [Gl] Flagging {n_low} points < {min_y:.2f}")
                self.df.loc[low_mask, 'error_flag'] |= self.error_masks['Gl: Value Low']
            if max_y is not None:
                high_mask = op_mask & (self.df[var_col] > max_y)
                n_high = high_mask.sum()
                if n_high > 0: self.log(f"  [Gl] Flagging {n_high} points > {max_y:.2f}")
                self.df.loc[high_mask, 'error_flag'] |= self.error_masks['Gl: Value High']

        gl_mask_val = 128 | 256
        if self.verbose:
            gl_hits = (self.df['error_flag'] & gl_mask_val) != 0
            self.log(f"Phase 2 Complete: {gl_hits.sum()} points flagged as Global Outlier.")

        # --- PHASE 3: SPATIAL ---
        self.log("Phase 3: Detecting Local/Spatial Anomalies...")
        ...
        if self.verbose:
            lo_hits = (self.df['error_flag'] & lo_mask_val) != 0
            self.log(f"Phase 3 Complete: {lo_hits.sum()} points flagged as Local Outlier.")
    ...
    def _apply_simple_annotation(self):
        ...
        op_flag_mask = 1 | 2 | 4 | 8 | 16 | 32 | 64 | 1024 | 2048 | 8192 | 16384
        ...
        simple[still_clean & ((flags & gl_mask) != 0)] = "Global Outlier"
        ...
        simple[assign_local] = "Local Outlier"
    ...
    def apply_local_std_filter(self, var_col, sigma, cell_multiplier):
        ...
        n_abnormal = (abnormal & mask_clean).sum()
        if n_abnormal > 0:
            self.log(f"  [Local] Flagging {n_abnormal} points outside {sigma} sigma.")
...
class SpatialDataFilteringAlgorithm(QgsProcessingAlgorithm):
    ...
    # Operational Filter Toggles
    FILTER_MANEUVERS = 'FILTER_MANEUVERS'
    ...
    def initAlgorithm(self, config=None):
        ...
        # --- FEATURE SELECTION (Which filters to run) ---
        self.addParameter(QgsProcessingParameterBoolean(self.FILTER_MANEUVERS, 'Filter Maneuvers (Turns/Short)', defaultValue=True))
        self.addParameter(QgsProcessingParameterBoolean(self.FILTER_OVERLAP, 'Filter Overlaps (Grid)', defaultValue=True))
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        try:
            ...
            pass
        cleaner = DataCleaner(df, crs=crs, feedback=feedback, verbose=True)
        ...
        filters = {
            ...
            'filter_maneuvers': self.parameterAsBool(parameters, self.FILTER_MANEUVERS, context),
        ...
        # Report Swath info
        swath = cleaner.calculate_swath_width()
        feedback.pushInfo(f"[Pipeline] Processing with Swath: {swath:.2f}m")
